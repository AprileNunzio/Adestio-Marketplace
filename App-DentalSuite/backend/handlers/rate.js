...
function scadenziario(payload = {}) {
    ...
    const aperte = rate.findAll({}).filter(riga => riga.stato !== dominio.STATO_PAGATA);
    const anagrafiche = new Map(pazienti.findAll({ includeArchived: true }).map(p => [p.id, p]));
    const righe = aperte
        .filter(riga => !payload.dal || riga.data_scadenza >= payload.dal)
        .filter(riga => !payload.al || riga.data_scadenza <= payload.al)
        .map(riga => ({
            ...riga,
