...
async function main() {
    ...
    const { MODULI } = await import(pathToFileURL(path.join(APP_ROOT, 'ui', 'routes.js')).href);
    ...
    await attendiRender();
    verifica(`Hub renderizzato con ${MODULI.length} moduli`,
        radice.querySelectorAll('.ds-card').length === MODULI.length,
    ...
    const sezioni = MODULI.map(modulo => modulo.id);
    ...
    verifica('Staff con 4 schede (collaboratori, turni, accordi, liquidazioni)',
        schedeStaff.length === 4, `${schedeStaff.length}`);
    ...
    verifica('Contabilità con 5 schede', schedeContabili.length === 5, `${schedeContabili.length}`);
