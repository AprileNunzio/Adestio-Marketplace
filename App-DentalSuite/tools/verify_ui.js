...
function classiDichiarate() {
    const cartella = path.join(APP_ROOT, 'css');
    if (!fs.existsSync(cartella)) return new Set();
    return fs.readdirSync(cartella)
        .filter(nome => nome.endsWith('.css'))
        .reduce((insieme, nome) => {
            const sorgente = fs.readFileSync(path.join(cartella, nome), 'utf8');
            [...sorgente.matchAll(/\.(ds-[a-z0-9_-]+)/g)].forEach(trovato => insieme.add(trovato[1]));
            return insieme;
        }, new Set());
}

function verificaStili() {
    const dichiarate = classiDichiarate();
    const usate = new Map();

    scanner.collectFiles(path.join(APP_ROOT, 'ui'), '.js').forEach(file => {
        const sorgente = fs.readFileSync(file, 'utf8').replace(/--ds-[a-z0-9_-]+/g, '');
        [...sorgente.matchAll(/ds-[a-z0-9_-]+/g)].forEach(trovato => {
            const nome = trovato[0];
            if (nome.endsWith('-')) return;
            if (!usate.has(nome)) usate.set(nome, path.relative(APP_ROOT, file));
        });
    });

    let senzaStile = 0;
    usate.forEach((origine, nome) => {
        if (dichiarate.has(nome)) return;
        senzaStile += 1;
        segnala('STILE', `${origine}: la classe "${nome}" non è definita in nessun foglio di css/`);
    });

    return { usate: usate.size, dichiarate: dichiarate.size, senzaStile };
}
...
function main() {
    ...
    const stili = verificaStili();

    console.log(`Moduli frontend analizzati: ${file.length} | viste registrate: ${registrate}`);
    console.log(`Classi di stile usate: ${stili.usate} | dichiarate in css/: ${stili.dichiarate}`);
