...
module.exports = async function scenario({ chiama, verifica, assertOk, assertKo, host, contesto }) {
    ...
    const situazione = assertOk('Situazione economica del paziente',
        await chiama('economia.paziente', { paziente_id: paziente.id }));
    verifica('Il saldo non produce mai numeri negativi da mostrare',
        situazione.a_debito >= 0 && situazione.credito >= 0 && situazione.valore_saldo >= 0,
        `debito ${situazione.a_debito} · credito ${situazione.credito} · mostrato ${situazione.valore_saldo}`);
    verifica('L etichetta del saldo corrisponde al segno',
        (situazione.a_debito > 0 && situazione.etichetta_saldo === 'Da incassare')
        || (situazione.credito > 0 && situazione.etichetta_saldo === 'Credito del paziente')
        || (situazione.in_pari && situazione.etichetta_saldo === 'In pari'),
        `${situazione.etichetta_saldo} · ${situazione.valore_saldo}`);
    verifica('Il dovuto nasce dalle prestazioni eseguite, non dai preventivi',
        situazione.saldo === Math.round((situazione.eseguito - situazione.incassato) * 100) / 100,
        `eseguito ${situazione.eseguito} incassato ${situazione.incassato} saldo ${situazione.saldo}`);
