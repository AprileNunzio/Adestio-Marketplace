...
import { generaPer, generaArretrati } from './derivate.js';
...
export default {
    rendi: async ({ paziente }) => {
        ...
        const disegna = async () => {
            ...
            const arretrati = righe.filter(riga => riga.derivate_stato === 'da_generare').length;
            ...
            const acquisisci = async () => {
                ...
                if (!dati) return;
                await disegna();
                const nuova = elenco(await call('allegati.daDerivare', { paziente_id: paziente.id, dimensione: 1 }));
                if (nuova.length > 0) {
                    const esitoDerivate = await generaPer(nuova[0].id);
                    if (esitoDerivate.generate) {
                        esito({ success: true }, `Versioni di consultazione create: ${dimensione(esitoDerivate.peso_visione)} invece di ${dimensione(esitoDerivate.peso_originale)}`);
                    }
                    await disegna();
                }
            };

            const ottimizzaArretrati = async () => {
                const esitoLotto = await generaArretrati(paziente.id, 20);
                esito({ success: true }, esitoLotto.elaborate > 0
                    ? `${esitoLotto.elaborate} referti ottimizzati · ${dimensione(esitoLotto.risparmio)} da trasmettere in meno`
                    : 'Nessun referto da ottimizzare');
                await disegna();
            };
            ...
            rimpiazza(contenitore, pannello({
                ...
                azioni: puoCaricare
                    ? [
                        arretrati > 0
                            ? bottone({
                                etichetta: `Ottimizza ${arretrati} referti`,
                                simbolo: 'compress',
                                variante: 'ghost',
                                onClick: ottimizzaArretrati
                            })
                            : null,
                ...
                colonne: [
                    ...
                    { titolo: 'Dimensione', numerica: true, rendi: riga => dimensione(riga.file_size) },
                    {
                        ...
                        rendi: riga => {
                            if (riga.derivate_stato === 'pronte') {
                                return distintivo(`WebP ${riga.larghezza || '?'}×${riga.altezza || '?'}`, 'success');
                            }
                            if (riga.derivate_stato === 'da_generare') return distintivo('da ottimizzare', 'warning');
                            return distintivo('solo originale', 'neutral');
                        }
                    },
                    {
                        titolo: '',
