...
import { pannelloRicerca } from './agenda/ricerca.js';
import { pannelloPanoramica } from './agenda/panoramica.js';
            ...
            disegna: ({ appuntamenti, poltrone }, aggiorna) => {
                ...
                return [
                    ...
                    pannelloPanoramica({
                        giorno,
                        onApri: voce => apri({
                            medico_id: voce.staff_id,
                            poltrona_id: poltrone[0] ? poltrone[0].id : '',
                            data_ora_inizio: fmt.inizioGiornata(giorno)
                                + (voce.slot[0] ? voce.slot[0].inizio : 9 * 60) * 60000,
                            durata_minuti: 30,
                            stato: 'programmato'
                        })
                    }).nodo,
                    pannelloRicerca({
                        onApri: apri,
                        onCartella: riga => naviga('paziente', { id: riga.paziente_id }),
                        onGiorno: riga => {
